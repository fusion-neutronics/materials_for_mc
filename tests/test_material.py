...
def test_material_reaction_mts_lithium():
    ...
    expected = sorted(['2',
        '102', '103', '104', '105', '16', '203', '204', '205', '207', '24', '25', '301', '444',
...
def test_hierarchical_mt3_generated_for_li6():
    ...
    mat.temperature = "294"
    energies, xs_dict = mat.calculate_macroscopic_xs_neutron(['3'])
    # MT=3 should be present and non-empty
    assert '3' in xs_dict, "MT=3 should be generated by sum rule for Li6"
    xs_mt3 = xs_dict['3']
    assert isinstance(xs_mt3, (list, tuple)), "MT=3 cross section should be a list or tuple"
    assert len(xs_mt3) > 0, "MT=3 cross section should not be empty"
    # Optionally: check that MT=3 is the sum of its children
    from materials_for_mc import sum_rules
    children = sum_rules()[3]
    sum_children = [0.0] * len(xs_mt3)
    for child in children:
        child_str = str(child)
        if child_str in xs_dict:
            xs_child = xs_dict[child_str]
            sum_children = [a + b for a, b in zip(sum_children, xs_child)]
    tol = 1e-10
    for a, b in zip(xs_mt3, sum_children):
        assert abs(a - b) < tol, "MT=3 cross section should equal sum of children"
